...
if (!userSql.includes("'qc_manager'")) {
  db.exec(`
    ...
    INSERT INTO users_v2 (
      id,username,password_hash,role,display_name,active,created_at
    )
    SELECT
      id,
      username,
...
db.prepare(`
  ...
  WHERE status IS NULL
     OR status NOT IN ('Pending QC','Qualified','Unqualified')
`).run();
...
function ensureInitialAdmin() {
  ...
  console.log(
    `Initial admin created: ${username}. Change the password after first login.`
  );
}
...
function audit(userId, action, leadId = null) {
  db.prepare(`
    INSERT INTO audit_log (user_id,action,lead_id)
    VALUES (?,?,?)
...
function validateLead(b) {
  ...
  if (!["Callback", "Appointment"].includes(clean(b.product_type))) {
    return "Invalid product type";
  }

  if (!["No Solar", "Has Solar"].includes(clean(b.solar_status))) {
    return "Invalid solar status";
  }

  if (!/^[0-9 +()\-]{7,20}$/.test(clean(b.mobile_number))) {
    return "Invalid mobile number";
  }

  if (!/^[0-9]{4}$/.test(clean(b.postcode))) {
    return "Postcode must be 4 digits";
  }

  if (!["Yes", "No"].includes(clean(b.free_standing))) {
    return "Invalid Free Standing value";
  }

  if (!["Yes", "No"].includes(clean(b.under_75))) {
  ...
  if (
    ...
    (
      ...
      !clean(b.panels_age) ||
      !["Yes", "No"].includes(clean(b.battery))
    )
  ) {
...
// Agent: create lead.
// The mobile number is never returned by any agent endpoint.
...
app.get("/api/qc/leads", requireRole("qc_manager"), (req, res) => {
  ...
  if (
    status &&
    ["Pending QC", "Qualified", "Unqualified"].includes(status)
  ) {
    sql += " WHERE l.status=?";
    params.push(status);
  }
...
app.get("/api/qc/stats", requireRole("qc_manager"), (req, res) => {
  ...
  res.json({
    ...
    total: get("SELECT COUNT(*) n FROM leads")
  });
});
...
app.patch(
  ...
  (req, res) => {
    ...
    const comment = clean(req.body.qc_comment);

    if (!["Qualified", "Unqualified"].includes(status)) {
      return res.status(400).json({
        error: "QC status must be Qualified or Unqualified"
      });
    }

    if (!comment) {
      return res.status(400).json({
        error: "QC Comment is required"
      });
    }
...
app.get("/api/admin/leads", requireAdmin, (req, res) => {
  ...
  let sql = `
    ...
    FROM leads l
    JOIN users u ON u.id=l.agent_id
    LEFT JOIN users q ON q.id=l.qc_by
    WHERE 1=1
  `;
...
app.get("/api/admin/stats", requireAdmin, (req, res) => {
  ...
  const agents = db.prepare(`
    SELECT
      ...
      SUM(
        CASE
          WHEN l.status='Unqualified' THEN 1
          ELSE 0
        END
      ) unqualified
    FROM users u
