...
const SESSION_SECRET =
  process.env.SESSION_SECRET || "CHANGE_THIS_SESSION_SECRET_IN_PRODUCTION";

app.set("trust proxy", 1);

app.use(cors({
  origin: "https://shivamofficialgoyal-lgtm.github.io",
  credentials: true
}));
...
function validateLead(body) {
  ...
  if (!["Callback", "Appointment"].includes(body.product_type)) {
    return "Invalid product type";
  }

  if (!["No Solar", "Has Solar"].includes(body.solar_status)) {
    return "Invalid solar status";
  }

  if (!/^[0-9 +()\-]{7,20}$/.test(body.mobile_number)) {
    return "Invalid mobile number";
  }

  if (!/^[0-9]{4}$/.test(body.postcode)) {
    return "Postcode must be 4 digits";
  }

  if (!["Yes", "No"].includes(body.free_standing)) {
    return "Invalid Free Standing value";
  }

  if (!["Yes", "No"].includes(body.under_75)) {
    return "Invalid Under 75 value";
  }

  if (body.solar_status === "Has Solar") {
    if (
      !clean(body.number_of_panels) ||
      !clean(body.panels_age) ||
      !["Yes", "No"].includes(body.battery)
    ) {
...
app.get("/api/admin/leads", requireAdmin, (req, res) => {
  const rows = db.prepare(`
    SELECT
      leads.*,
      users.username AS agent_username,
...
app.get("/api/admin/stats", requireAdmin, (req, res) => {
  ...
  const appointments = db
    .prepare("SELECT COUNT(*) AS n FROM leads WHERE product_type='Appointment'")
    .get().n;

  const callbacks = db
    .prepare("SELECT COUNT(*) AS n FROM leads WHERE product_type='Callback'")
    .get().n;

  const noSolar = db
    .prepare("SELECT COUNT(*) AS n FROM leads WHERE solar_status='No Solar'")
    .get().n;

  const hasSolar = db
    .prepare("SELECT COUNT(*) AS n FROM leads WHERE solar_status='Has Solar'")
    .get().n;
  ...
  const agents = db.prepare(`
    ...
    FROM users
    LEFT JOIN leads ON users.id = leads.agent_id
...
app.get("/api/health", (req, res) => {
  res.json({
    ok: true,
    message: "Solar CRM backend is running"
  });
});
...
app.listen(PORT, () => {
  console.log(`Solar CRM backend running on port ${PORT}`);
});
